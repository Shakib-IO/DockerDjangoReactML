...
const SESSION_DUARTION = settings.SESSION_DUARTION
...
export const authLogin = (username , password) =>{
    return dispatch => {
        ...
        .then(res => {
            ...
            const expirationDate =  new Date(new Date().getTime()+ SESSION_DUARTION );
            localStorage.setItem('token' , token);
            localStorage.setItem('expirationDate', expirationDate);
            dispatch(suthSuccess(token));
            dispatch(authCheckTimeout(SESSION_DUARTION));
        })
        .catch(err => {
    ...
    return dispatch => {
        ...
        if (token === undefined){
            ...
            if (expirationDate <= new Date()){
                ...
                dispatch(authCheckTimeout(expirationDate.getItem() - new Date().getTime()))
            }
        }
    }
}